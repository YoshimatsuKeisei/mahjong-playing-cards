...
| Deckout | `deckoutResult` | Implemented Phase 5 | None known | Allow empty-deck draw to reach reducer deckout result; sync match state | Public deckout result, no deck contents | Result UI | `online-round-deckout` |
| Round end | `matchState.ts syncMatchGameState` | Implemented Phase 5 | None known | Server owns `MatchState`; sync when reducer reaches `result` | Masked `matchState.gameState` per viewer; public history/scores | Existing ResultScreen | Phase 5 focused specs |
| Next round / room kept | `matchState.ts advanceRound` | Implemented Phase 5 | Host-only next round for now | Keep room/playerId/seat, advance server match, remap player ids/names | Fresh masked game state, cumulative history retained | Result next-round button emits `nextRound` | `online-next-round` |
| Score calculation | `scoring.ts`, `matchState.ts` | Implemented Phase 5 | None known | Use existing round score/raw score/deduction helpers through match sync | Public result and cumulative scores; hidden hands safe | Result/standings UI | round tsumo/ron/double ron/deckout specs |
