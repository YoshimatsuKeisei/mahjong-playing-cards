...
export default function ResultScreen({
  ...
  const [animateStandingsToCurrent, setAnimateStandingsToCurrent] =
    useState(false);
  const [resultScroller, setResultScroller] = useState<HTMLDivElement | null>(
    null,
  );
  ...
  const displayMode: ScoreDisplayMode =
    scoreDisplayMode ?? (useRawScore ? "targetScore" : "score");
  const isDeckout = result.winType === "deckout";
  const ronResults =
    result.winType === "ron"
      ? (result.ronResults ?? [singleRonResult(result)])
      : [];
  const nextRound = currentRound + 1;
  const canShowNextRound = Boolean(
    onNextRound && (totalRounds === undefined || currentRound < totalRounds),
  );
  const canShowStandings = Boolean(currentStandings && canShowNextRound);
  const winTypeLabel =
    result.winType === "tsumo"
      ? "ツモ"
      : result.winType === "ron"
        ? "ロン"
        : "流局";
  ...
  useEffect(() => {
    ...
    playCharacterVoice(
      playerAvatarId,
      winnerIndexes.includes(selfIndex) ? "victory" : "defeat",
    );
  ...
  const updateResultScrollBar = () => {
    ...
    const thumbTop =
      (resultScroller.scrollTop / maxScrollTop) * (100 - thumbHeight);
  ...
  const scrollResultByTrackPosition = (clientY: number) => {
    ...
    const maxScrollTop =
      resultScroller.scrollHeight - resultScroller.clientHeight;
    const thumbHeightPx =
      (resultScrollBar.thumbHeight / 100) * trackRect.height;
  ...
  const handleResultScrollTrackPointerDown = (
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(
      () => setAnimateStandingsToCurrent(true),
      120,
    );
  ...
  return (
    <main className="screen result-screen" data-testid="result-screen">
      <section
        className="result-panel result-board"
        onWheel={handleResultBoardWheel}
      >
        <div
          className="result-board-scroll"
          onScroll={updateResultScrollBar}
          ref={setResultScroller}
        >
          <div className="result-round-title">{currentRound}回戦</div>
          <h1
            className="result-board-title result-pop-item"
          ...
          <div className="player-result-list" data-testid="result-player-list">
            {state.players.map((player, index) => {
              const breakdown = buildPlayerBreakdown(
                state,
                result,
                player,
                index,
              );
              const hasMelds = breakdown.melds.length > 0;
                ...
                >
                  <div className="player-result-profile">
                    <div className="player-result-head">
                      <strong className="result-player-name-badge">
                        {getResultDisplayPlayerName(player.name)}
                      </strong>
                      {label ? (
                        <em className="result-status-badge">{label}</em>
                      ) : (
                        <em
                          className="result-status-badge is-empty"
                          aria-hidden="true"
                        />
                      )}
                    </div>
                    <img
                      ...
                      src={getResultCharacterSrcForPlayer(
                        state,
                        index,
                        playerAvatarId,
                      )}
                  ...
                  <div className="player-result-cards">
                    <div
                      className={`result-hand-breakdown ${hasMelds ? "" : "no-melds"}`}
                      aria-label={`${getResultDisplayPlayerName(player.name)}の手札内訳`}
                    >
                      <div
                        className={`result-meld-column ${hasMelds ? "" : "is-empty"}`}
                      >
                        <span>できた役</span>
                          ...
                          {!hasMelds ? (
                            ...
                            breakdown.melds.map((meld, meldIndex) => (
                              <div
                                ...
                                key={`${player.id}-meld-${meldIndex}-${meld.map((card) => card.id).join("-")}`}
                              >
                      ...
                      <div className="result-rest-column">
                        ...
                        <div className="result-card-group">
                          {breakdown.remainder.length === 0 ? (
                            ...
                            sortCardsForDisplay(breakdown.remainder).map(
                              (card) => (
                                <PlayingCard
                                  card={card}
                                  compact
          ...
          <section
            className="score-result-panel result-pop-item"
            style={{ animationDelay: `${0.4 + state.players.length * 0.4}s` }}
          >
            ...
            <div className="score-result-main">
              ...
              {displayMode !== "startingPoints" && (
                <FormulaExpression
                  parts={buildScoreFormulaPartsForMode(
                    state,
                    result,
                    displayMode,
                  )}
            ...
            <div className="score-final">
              <span>
                {displayMode === "startingPoints" ? "持ち点" : "得点"}
              </span>
              <strong>
                {getDisplayFinalScore(
                  result,
              ...
              <div className="formula-breakdown">
                {ronResults.map((item) => (
                  <div className="formula-row" key={item.winnerIndex}>
                    <span>
                      {getResultDisplayPlayerName(
                        state.players[item.winnerIndex].name,
                      )}
                    </span>
                    <FormulaExpression
                      parts={buildScoreFormulaPartsForMode(
              ...
              <div className="formula-breakdown">
                {buildStartingPointDeductionRows(state, result).map((row) => (
                  <div
                    ...
                    key={`${row.playerName}-${row.parts.map((part) => part.value).join("-")}`}
                  >
          ...
          <section
            className="result-winner-call result-pop-item"
            style={{ animationDelay: `${0.8 + state.players.length * 0.4}s` }}
          >
            ...
            <div className="result-winner-line">
              ...
              {canShowStandings && (
                <button
                  type="button"
                  className="standings-toggle-button"
          ...
          <div
            className="result-actions result-pop-item"
            style={{ animationDelay: `${1.8 + state.players.length * 0.4}s` }}
          >
            {canShowNextRound && (
              <button
                type="button"
        ...
        <div
          className={`result-custom-scrollbar ${resultScrollBar.enabled ? "visible" : ""}`}
          aria-hidden="true"
        >
          <div
            className="result-custom-scrollbar-track"
            onPointerDown={handleResultScrollTrackPointerDown}
          >
            <div
      ...
      {parts.map((part, index) =>
        part.label ? (
          <span
            ...
            key={`${part.value}-${part.label}-${index}`}
          >
...
function getResultViewerPlayerIndex(state: GameState) {
  if (state.viewerPlayerId) {
    return state.players.findIndex(
      (player) => player.id === state.viewerPlayerId,
    );
  }
...
function getResultCharacterSrcForPlayer(
  state: GameState,
  playerIndex: number,
  playerAvatarId: string,
) {
  return getAvatarById(
    playerIndex === getResultViewerPlayerIndex(state)
      ? playerAvatarId
      : "home-character-1",
  ).imageSrc;
}
...
function getResultLabel(result: GameResult, playerIndex: number) {
  ...
  const isWinner =
    result.ronResults?.some((item) => item.winnerIndex === playerIndex) ??
    result.winnerIndex === playerIndex;
  if (isWinner) return "勝者";
  if (result.winType === "ron")
    return result.discarderIndex === playerIndex ? "敗者" : "";
  if (result.winType === "tsumo")
    return result.winnerIndex === playerIndex ? "勝者" : "敗者";
  return "";
}
...
function getResultRowTone(result: GameResult, playerIndex: number) {
  ...
  if (result.winType === "ron")
    return result.discarderIndex === playerIndex ? "loser" : "normal";
  if (result.winType === "tsumo")
    return result.winnerIndex === playerIndex ? "winner" : "loser";
  return "normal";
}

function getDisplayPlayerLoss(result: GameResult, playerIndex: number) {
  const ronResult = result.ronResults?.find(
    (item) => item.winnerIndex === playerIndex,
  );
  return (
    ronResult?.score.playerLosses[playerIndex] ??
    result.score.playerLosses[playerIndex] ??
    0
  );
}
...
function buildPlayerBreakdown(
  ...
  const best = findBestDisplayMelds(
    player.hand,
    Math.max(0, 3 - player.openMelds.length),
  );
  return {
    melds: [
      ...player.openMelds.map(sortCardsForDisplay),
      ...best.melds.map(sortCardsForDisplay),
    ],
    remainder: sortCardsForDisplay(best.remainder),
  };
}

function getPlayerWinningResult(
  result: GameResult,
  player: Player,
  playerIndex: number,
): WinningResult | null {
...
function getWinningRemainder(cards: Card[], winningResult: WinningResult) {
  ...
  if (
    winningResult.keyCard &&
    !afterMelds.some((card) => card.id === winningResult.keyCard?.id)
  ) {
...
function findBestDisplayMelds(
  cards: Card[],
  meldsToRemove: number,
): HandBreakdown {
...
function chooseBetterBreakdown(
  current: HandBreakdown | null,
  candidate: HandBreakdown,
): HandBreakdown {
  ...
  return sumPenalty(candidate.remainder) < sumPenalty(current.remainder)
    ? candidate
    : current;
}
...
function getDisplayFinalScore(
  ...
  if (mode === "startingPoints") {
    const deduction = calculateStartingPointDisplayDeduction(
      result,
      playerCount,
    );
    return deduction > 0 ? `-${deduction}` : "0";
  }
  return mode === "targetScore"
    ? calculateRawWinnerScore(result)
    : result.score.winnerScore;
}

function buildScoreFormulaPartsForMode(
  state: GameState,
  result: GameResult,
  mode: ScoreDisplayMode,
): FormulaPart[] {
  ...
  if (result.winType === "ron" && result.discarderIndex !== null) {
    ...
    return [
      { value: "(" },
      {
        value: String(discarderLoss),
        label: `${getResultDisplayPlayerName(state.players[result.discarderIndex].name)}の失点`,
      },
      { value: "-" },
      {
        value: String(winnerLoss),
        label: `${getResultDisplayPlayerName(state.players[result.winnerIndex].name)}の失点`,
      },
      { value: ")=" },
  ...
  const loserLosses = result.score.playerLosses.filter(
    (_, index) => index !== result.winnerIndex,
  );
  const divisor = Math.max(1, loserLosses.length);
  const score = calculateRawTsumoScoreFromLosses(
    result.score.playerLosses,
    result.winnerIndex,
  );
  return [
    { value: "((" },
...
function buildStartingPointDeductionRows(
  state: GameState,
  result: GameResult,
): DeductionRow[] {
  if (result.winType === "deckout") return [];
  if (result.winType === "ron" && result.discarderIndex !== null) {
    ...
    const discarderName = getResultDisplayPlayerName(
      state.players[discarderIndex].name,
    );
    ...
    if (ronResults.length > 1) {
      const winnerLosses = ronResults.map(
        (item) => item.score.playerLosses[item.winnerIndex] ?? 0,
      );
      const winnerAverageLoss = Math.round(
        winnerLosses.reduce((sum, loss) => sum + loss, 0) / winnerLosses.length,
      );
      const deduction = calculateRawScoreFromLosses(
        discarderLoss,
        winnerAverageLoss,
      );
      return [
    ...
    return [
      {
        ...
        parts: [
          ...
          {
            value: String(winnerLoss),
            label: `${getResultDisplayPlayerName(state.players[winnerIndex].name)}の失点`,
          },
          { value: "-" },
          { value: String(discarderLoss), label: `${discarderName}の失点` },
          { value: ")=" },
  ...
  const loserLosses = result.score.playerLosses.filter(
    (_, playerIndex) => playerIndex !== result.winnerIndex,
  );
  const loserAverageLoss =
    loserLosses.length > 0
      ? Math.round(
          loserLosses.reduce((sum, loss) => sum + loss, 0) / loserLosses.length,
        )
      : 0;
  const winnerLoss = result.score.playerLosses[result.winnerIndex] ?? 0;
  const winnerName = getResultDisplayPlayerName(
...
function calculateStartingPointDisplayDeduction(
  result: GameResult,
  playerCount: number,
) {
  if (result.winType === "deckout") return 0;
    ...
    const winnerLoss =
      ronResults.length > 1
        ? Math.round(
            ronResults.reduce(
              (sum, item) =>
                sum + (item.score.playerLosses[item.winnerIndex] ?? 0),
              0,
            ) / ronResults.length,
          )
...
function buildScoreFormulaParts(
  state: GameState,
  result: GameResult,
): FormulaPart[] {
  const winnerLoss = result.score.playerLosses[result.winnerIndex] ?? 0;
